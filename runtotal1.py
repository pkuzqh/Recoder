...
for i in tqdm(range(int(len(lst) / 10) + 1)):
            ...
            for j in range(10):
                if 10 * i + j >= len(lst):
                    continue
                cardn =int(j / 5)
                p = subprocess.Popen("CUDA_VISIBLE_DEVICES="+str(card[cardn]) + " python3 testDefect4j1.py " + lst[10 * i + j], shell=True)#subprocess.Popen("CUDA_VISIBLE_DEVICES="+str(card) + " python3 testDefect4j.py " + lst[12 * i + j], stderr=subprocess.DEVNULL, stdout=subprocess.PIPE, shell=True)#subprocess.run(["python3", "run.py"])
